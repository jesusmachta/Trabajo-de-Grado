...
def get_most_visited_category_historical(empresa: str) -> Dict[str, Any]:
    """
    Obtiene la categoría de producto más visitada históricamente desde la colección Estadisticas.
    
    Args:
        empresa: Identificador de la empresa para la que se obtienen las estadísticas
        
    Returns:
        Dictionary containing the most visited category historical data
    ...
    try:
        ...
        if not data or data.get("category") == "":
            return {"most_visited_category": None, "count": 0}
        ...
        logger.error(f"Error fetching historical most visited category for company '{empresa}': {str(e)}")
